...
1. **Colors, type, spacing, radii come ONLY from the design tokens** in `docs/design/tokens.json` (74 light + 7 night). Never hardcode a hex value or a raw pixel spacing in a widget — map tokens into the Flutter theme in `packages/sahra_design_system` and reference the theme.
