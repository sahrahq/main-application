...
| Anything visual: a screen, a component, colors, spacing, theming | `docs/design/DESIGN-RULES.md` (design contract), then the specific HTML reference in `docs/design/ui_kits/app/*.html` |
...
1. **Colors, type, spacing, radii come ONLY from the design tokens** in `docs/design/tokens.json` (73 light + 7 night). Never hardcode a hex value or a raw pixel spacing in a widget — map tokens into the Flutter theme in `packages/sahra_design_system` and reference the theme.
...
5. **Match the HTML reference precisely.** When the written rules are ambiguous, open the actual HTML file (`docs/design/ui_kits/app/<screen>.html`) and read the real values — it is the source of pixel truth.
...
2. **Design foundation** (Dev B), in this exact order — same order the design system was built:
   ...
   2. **Shared components** — build the 32 components from `docs/design/ui_kits/` as reusable widgets in `sahra_design_system`.
   3. **Screens** — build each screen from its HTML reference, composing the components, wired to the real API via `sahra_api_client`.

One screen per Claude Code prompt — e.g. *"Implement the Discover screen in Flutter, matching `docs/design/ui_kits/app/discover.html` exactly, per docs/design/DESIGN-RULES.md, using tokens/components from packages/sahra_design_system."* This keeps fidelity high; "build the whole app" does not.
